...
const SpeakingHeadIcon: React.FC<{ className?: string; size?: number }> = ({ className, size }) => (
    <span role="img" aria-label="Belajar Iqro" className={className} style={{ fontSize: size }}>🗣️</span>
);
    ...
    return (
        <header className="sticky top-0 bg-white/80 dark:bg-dark-blue/80 backdrop-blur-md z-30 flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800 shadow-sm">
            <div className="flex items-center space-x-4">
                <button onClick={onMenuClick} aria-label="Buka Menu" className="md:hidden p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800"><Menu size={20}/></button>
            ...
            <div className="flex items-center gap-2">
                ...
                <Link to="/" className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Home"><HomeIcon size={20}/></Link>
                <div className="relative">
                    <button onClick={onSettingsClick} aria-label="Pengaturan" className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800"><Cog size={20}/></button>
    ...
    return (
        <aside className={`fixed inset-y-0 left-0 w-64 bg-white dark:bg-dark-blue-card border-r border-gray-100 dark:border-gray-800 z-40 transform transition-transform duration-300 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
            <div className="p-6 flex items-center justify-between">
                ...
                <button onClick={() => setSidebarOpen(false)} aria-label="Tutup Sidebar" className="md:hidden"><X/></button>
            </div>

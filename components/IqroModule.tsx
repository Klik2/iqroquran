        ...
        {IQRO_DATA.map((lvl) => {
          ...
          return (
            <button 
              ...
              className={`relative flex flex-col items-center gap-4 p-8 rounded-[3rem] border-4 transition-all group active:scale-95 shadow-xl ${activeLevelId === lvl.id && viewMode !== 'hijaiyah' ? colorClasses[lvl.color] + ' -translate-y-2' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 hover:border-emerald-400'}`}
            >
              <div className={`w-16 h-16 rounded-3xl flex items-center justify-center font-black text-2xl shadow-inner ${activeLevelId === lvl.id && viewMode !== 'hijaiyah' ? 'bg-white/20' : lightColorClasses[lvl.color]}`}>
                {lvl.id}
              </div>
              <span className={`font-black text-sm tracking-tight ${activeLevelId === lvl.id && viewMode !== 'hijaiyah' ? 'text-white' : 'text-slate-900 dark:text-white'}`}>{lvl.title}</span>
              ...
              {progress === 100 && (
                <div className="absolute -top-3 -right-3 p-2 bg-amber-400 text-white rounded-full shadow-lg scale-110">
                  <Trophy size={20} />
                </div>
              )}
            </button>
      ...
      <div className="flex flex-wrap justify-center gap-4">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-3 px-10 py-4 rounded-full font-black uppercase tracking-widest transition-all shadow-lg active:scale-95 ${viewMode === mode.id ? colorClasses[activeLevel.color] : 'bg-white dark:bg-slate-800 text-slate-500 border-2 border-slate-100 dark:border-slate-700 hover:border-emerald-500'}`}
          >
        ...
        <button 
          ...
          className="flex items-center gap-3 px-6 py-4 rounded-full font-black uppercase tracking-widest bg-rose-100 dark:bg-rose-900/30 text-rose-600 hover:bg-rose-600 hover:text-white transition-all shadow-lg active:scale-95"
        >

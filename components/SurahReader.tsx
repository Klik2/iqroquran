...
const SurahReader: React.FC<ReaderProps> = ({ appLang, t }) => {
  ...
  if (error || !data || !Array.isArray(data) || data.length < 2) return (
    <div className="flex flex-col items-center justify-center h-full py-48 gap-8 px-6 text-center">
      ...
      <div className="flex gap-4">
        <button onClick={() => navigate('/mushaf')} className="px-8 py-3 bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-white rounded-2xl font-black hover:bg-slate-300 dark:hover:bg-slate-600 transition-all">{t.back}</button>
  ...
  return (
    <div className="max-w-4xl mx-auto pb-48 px-4 sm:px-6">
      ...
      {/* Instruction #5: Responsive Bismillah scaling */}
      {arabicEd.number !== 1 && arabicEd.number !== 9 && (
        <div className="text-center py-20 animate-in fade-in slide-in-from-top-4 duration-1000 flex justify-center">
           <p className="font-quran bismillah-text select-none font-bold text-slate-950 dark:text-white" dir="rtl">
            ...
            <div key={ayah.number} className="group relative animate-in fade-in slide-in-from-bottom-12 duration-700">
              <div className="flex flex-col gap-12">
                ...
                <div className="text-right">
                  <p 
                    className="font-quran leading-[2.6] tracking-wide w-full select-all font-bold text-slate-950 dark:text-white"
                ...
                {showLatin && (
                   <div className="text-right border-r-4 border-emerald-500/30 pr-6 mr-2">
                     <p className="latin-reading text-2xl leading-relaxed tracking-tight text-slate-950 dark:text-emerald-400">
                        {latinText}
                     </p>
                   </div>
                )}
